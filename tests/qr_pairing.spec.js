...
(0, test_1.test)('QR Code Generation Test', async ({ page }) => {
    ...
    await (0, test_1.expect)(page.locator('nav')).toBeVisible({ timeout: 60000 });
    // Open Pairing Modal
    await page.click('button:has-text("Pair Device")');
    await (0, test_1.expect)(page.locator('.modal-title')).toContainText('Direct Pairing', { timeout: 10000 });
    // Step 1: Initiator
    await page.click('button:has-text("1. I am the INITIATOR (QR)")');
    // Wait for "SCAN ME" step
    await (0, test_1.expect)(page.locator('h6:has-text("SCAN ME")')).toBeVisible({ timeout: 30000 });
    // Check for canvas
    ...
    const isCanvasBlank = await canvas.evaluate((canvas) => {
    ...
    (0, test_1.expect)(isCanvasBlank).toBe(false);
    console.log('QR Code canvas verified: NOT BLANK');
});
//# sourceMappingURL=qr_pairing.spec.js.map
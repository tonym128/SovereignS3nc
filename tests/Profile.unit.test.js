...
describe('ProfileModule Unit Tests', () => {
    ...
    beforeEach(async () => {
        ...
        await sov.init();
        profileModule = new Profile_1.ProfileModule(sov);
    });
    ...
    test('syncOtherProfiles should download and save followed profiles', async () => {
        ...
        const bobProfile = { userId: 'bob', name: 'Bob Smith', bio: 'Builder', updatedAt: Date.now() };
        ...
        jest.spyOn(sov, 'createRemote').mockImplementation((...args) => {
            const userId = args[0];
            if (userId === 'bob')
                return bobRemote;
...
//# sourceMappingURL=Profile.unit.test.js.map
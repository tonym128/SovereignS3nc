...
const MediaUtils_1 = require("../src/utils/MediaUtils");
...
describe('MediaUtils', () => {
    describe('compressImage', () => {
        it('should compress a large image in Node.js environment', async () => {
            ...
            // @ts-ignore
            const image = new jimp_1.Jimp({ width: 2000, height: 2000, color: 0xFF0000FF });
            const buffer = await image.getBuffer('image/png');
            const dataUrl = `data:image/png;base64,${buffer.toString('base64')}`;
            // Try to compress
            const targetSize = 10000;
            const result = await MediaUtils_1.MediaUtils.compressImage(dataUrl, targetSize);
        ...
        it('should handle invalid data URLs gracefully', async () => {
            ...
            const result = await MediaUtils_1.MediaUtils.compressImage(invalid, 100);
            expect(result).toBe(invalid);
        });
    });
});
//# sourceMappingURL=MediaUtils.unit.test.js.map
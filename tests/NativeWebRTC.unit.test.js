...
const NativeWebRTCTransport_1 = require("../src/adapters/NativeWebRTCTransport");
...
describe('Native WebRTC & PEX Logic', () => {
    it('should perform a manual handshake flow', async () => {
        const alice = new NativeWebRTCTransport_1.NativeWebRTCTransport('alice');
        const bob = new NativeWebRTCTransport_1.NativeWebRTCTransport('bob');
    ...
    it('WebRTCRemoteAdapter should relay signals (PEX)', () => {
        const aliceAdapter = new WebRTCRemoteAdapter_1.WebRTCRemoteAdapter('alice');
        const bobAdapter = new WebRTCRemoteAdapter_1.WebRTCRemoteAdapter('bob');
    ...
    it('WebRTCRemoteAdapter should handle incoming relayed signals', (done) => {
        const charlieAdapter = new WebRTCRemoteAdapter_1.WebRTCRemoteAdapter('charlie');
        const signal = { type: 'offer', sdp: 'test' };
        ...
        charlieAdapter.handleMessage(incomingMsg, { send: jest.fn() });
    });
});
//# sourceMappingURL=NativeWebRTC.unit.test.js.map
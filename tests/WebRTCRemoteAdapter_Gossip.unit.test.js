...
describe('WebRTCRemoteAdapter Gossip Scaling and Safety', () => {
    test('should enforce maxPeers limit', () => {
        const peer = new WebRTCRemoteAdapter_1.WebRTCRemoteAdapter('main', '', { maxPeers: 3 });
    ...
    test('should respect TTL (hop limit)', async () => {
        // Chain: A -> B -> C -> D
        const peerA = new WebRTCRemoteAdapter_1.WebRTCRemoteAdapter('A', '', { ttl: 2 });
        const peerB = new WebRTCRemoteAdapter_1.WebRTCRemoteAdapter('B', '', { ttl: 2 });
        const peerC = new WebRTCRemoteAdapter_1.WebRTCRemoteAdapter('C', '', { ttl: 2 });
        const peerD = new WebRTCRemoteAdapter_1.WebRTCRemoteAdapter('D', '', { ttl: 2 });
    ...
    test('should deduplicate messages using msgId', async () => {
        // Loop: A -> B -> C -> A
        const peerA = new WebRTCRemoteAdapter_1.WebRTCRemoteAdapter('A');
        const peerB = new WebRTCRemoteAdapter_1.WebRTCRemoteAdapter('B');
        const peerC = new WebRTCRemoteAdapter_1.WebRTCRemoteAdapter('C');
        let aReceiveCount = 0;
        const originalHandleMessage = peerA.handleMessage.bind(peerA);
        peerA.handleMessage = (msgStr, source) => {
            aReceiveCount++;
...
//# sourceMappingURL=WebRTCRemoteAdapter_Gossip.unit.test.js.map
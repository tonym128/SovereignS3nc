...
async function loginUser(page, userId, password) {
    ...
    await (0, test_1.expect)(page.locator('nav')).toBeVisible({ timeout: 20000 });
}
(0, test_1.test)('Sovereign Social Multi-User Journey', async ({ browser }) => {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (await followBtn.isVisible() || await followingBtn.isVisible())
            break;
        console.log(`Sync attempt ${i + 1} for discovery...`);
    }
    if (await followBtn.isVisible()) {
        await followBtn.click();
        await (0, test_1.expect)(followingBtn).toBeVisible({ timeout: 10000 });
    }
    else {
    ...
    if (await aliceFollowBobBtn.isVisible()) {
        ...
        await (0, test_1.expect)(alicePage.locator('.list-group-item').filter({ hasText: bobId }).locator('button:has-text("Following")')).toBeVisible({ timeout: 10000 });
    }
    ...
    for (let i = 0; i < 5; i++) {
        await bobPage.click('button:has-text("Sync")');
        await bobPage.waitForTimeout(3000);
        if (await alicePost.isVisible())
            break;
    }
    await (0, test_1.expect)(alicePost).toBeVisible({ timeout: 5000 });
    await bobPage.click('button:has-text("Like")');
    await (0, test_1.expect)(bobPage.locator('button:has-text("Like (1)")')).toBeVisible();
    ...
    await (0, test_1.expect)(alicePage.getByTestId('message-input')).toBeVisible({ timeout: 10000 });
    // Send message
    ...
    for (let i = 0; i < 5; i++) {
        await bobPage.click('button:has-text("Sync")');
        await bobPage.waitForTimeout(3000);
        if (await msgBadge.isVisible())
            break;
    }
    await (0, test_1.expect)(msgBadge).toBeVisible({ timeout: 5000 });
    await (0, test_1.expect)(msgBadge).toHaveText('1');
    // Bob reads & replies
    await bobPage.getByTestId('nav-messages').click();
    await bobPage.getByTestId(`chat-item-${aliceId}`).click();
    await (0, test_1.expect)(bobPage.getByTestId('message-bubble').filter({ hasText: 'Hey Bob, Alice here!' })).toBeVisible();
    ...
    for (let i = 0; i < 10; i++) {
        ...
        if (await bobReply.isVisible())
            break;
    }
    await (0, test_1.expect)(bobReply).toBeVisible({ timeout: 5000 });
    ...
    await (0, test_1.expect)(alicePage.locator('.modal')).toBeHidden({ timeout: 10000 });
    ...
    for (let i = 0; i < 10; i++) {
        ...
        if (await threadedReply.isVisible())
            break;
    }
    await (0, test_1.expect)(threadedReply).toBeVisible({ timeout: 5000 });
    console.log("Multi-user journey completed successfully (excluding reset/restore).");
    await aliceContext.close();
    await bobContext.close();
});
//# sourceMappingURL=multi_user_browser.spec.js.map